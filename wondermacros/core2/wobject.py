...
class Class:
    ...
    def add_method(self,type,name,args=""):
        self.methods.append(Method(type,name,args))

    def add_override(self,name):
        self.override.append(name)
...
class Parser:
    ...
    def parse_method(self,line):
        matchObj = re.match( r'^\s+((\w+\s+)*\w+\**)\s+(\w+)\s*\(([^\)]*)\)', line )
        if (matchObj):
            self.klass.add_method(matchObj.group(1),matchObj.group(3),matchObj.group(4))
            return True
        else:
            return False
    ...
    def parse_override(self,line):
        ...
        if (matchObj):
            self.klass.add_override(matchObj.group(1))
            return True
        else:
            return False
    ...
    def parse_var(self,line):
        ...
        if (matchObj):
            self.klass.add_var(matchObj.group(1),matchObj.group(3))
            return True
        else:
            return False
